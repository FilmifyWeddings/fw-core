...
import {
  ...
  AlertCircle, Play, ExternalLink, Star, Compass, UserCheck,
  Eye, Clock, X
} from 'lucide-react';
...
export default function PublicMoodboardPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        // Open sections that already have data
        setExpandedSections({
          '01': true,
          '02': true,
          '03': true,
          '04': Array.isArray(mb.close_family_photos) && mb.close_family_photos.length > 0,
          '05': Array.isArray(mb.photo_references) && mb.photo_references.length > 0,
          '06': Array.isArray(mb.video_references) && mb.video_references.length > 0,
          '07': Array.isArray(mb.itinerary_schedule) && mb.itinerary_schedule.length > 0,
          '08': Array.isArray(mb.venue_locations) && mb.venue_locations.length > 0,
          '09': Array.isArray(mb.outfit_references) && mb.outfit_references.length > 0,
          '10': !!(mb.payment_contact?.name || mb.payment_contact?.phone),
        });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FBF9F5] text-slate-800 flex flex-col items-center justify-center p-6">
        <div className="relative flex items-center justify-center">
          <div className="w-16 h-16 rounded-full border-3 border-amber-200 border-t-amber-500 animate-spin" />
          <Heart className="w-6 h-6 text-amber-500 absolute animate-pulse" />
        </div>
        <p className="mt-5 text-xs font-bold tracking-widest text-amber-700 uppercase">
          Loading Your Wedding Mood Board...
        </p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#FBF9F5] text-slate-800 flex flex-col items-center justify-center p-6 text-center">
        <AlertCircle className="w-14 h-14 text-rose-500 mb-3" />
        <h1 className="text-xl font-serif text-slate-900 font-bold mb-2">Mood Board Link Invalid</h1>
        <p className="text-xs text-slate-500 max-w-sm mb-5">{error}</p>
        <button
          onClick={() => window.location.reload()}
          className="px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs rounded-full transition"
        >
          Try Again
        </button>
      </div>
  ...
  const coupleTitle = clientInfo?.name || 'Rohit Weds Priya';
  const studioName = studioInfo?.name || 'STUDIOCORE PHOTOGRAPHY';
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => saveMoodboardData(true)}
              disabled={saving || submitted}
              className="px-6 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-bold text-xs rounded-xl shadow-xs transition flex items-center gap-1.5 active:scale-95"
            >
              <Send className="w-3.5 h-3.5" />
