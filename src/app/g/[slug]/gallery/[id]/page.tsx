...
function StrictZeroKnowledgeGuestContent() {
  ...
  const handleFaceScanSubmit = async (data: {
    ...
    try {
      // Step A: Register Guest in CRM/Database
      try {
      ...
      // Step B: Send Selfie to Neural ArcFace Match Endpoint
      ...
      if (json.success && Array.isArray(json.photos)) {
        ...
        if (json.photos.length > 0) {
          ...
          // Strict Zero Match: Do NOT leak full photos
          setMatchedPhotos([]);
          setIsVerified(false);
          setNoMatchModalOpen(true);
        }
      } else {
        alert(json.error || 'Face search encountered an issue. Please try again.');
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F5] text-zinc-900 font-sans pb-32 select-none">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 -mt-6 relative z-10 space-y-8">
        ...
        {/* CASE A: STRICT ZERO-KNOWLEDGE LOCKED HERO (Default for Guests) */}
        ...
        {/* CASE B: MATCHED PHOTOS VIEW (Strictly Render ONLY Matched Photos) */}
      ...
      {/* ─────────────────────────────────────────────────────────────
          4. ZERO MATCH FALLBACK MODAL (Never leaks all photos)
      ───────────────────────────────────────────────────────────── */}
      {noMatchModalOpen && (
