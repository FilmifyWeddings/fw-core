...
export async function GET(request: NextRequest) {
  try {
    ...
    if (linkErr || !link || !link.is_active) {
      // Mock fallback if local/demo testing
    ...
    // 8. Fetch Current Month Attendance Records for Calendar View
    const startOfMonth = `${todayDate.substring(0, 7)}-01`;
    const { data: monthlyHistory } = await supabaseAdmin
      .from('attendance_records')
      .select('id, date, status, check_in_time, check_out_time, work_duration_minutes, late_minutes, overtime_minutes')
      .eq('member_id', link.member_id)
