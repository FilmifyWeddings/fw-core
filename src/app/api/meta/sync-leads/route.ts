    ...
    while (nextUrl && loopCount < 200) {
      ...
      const formattedBatch = leadsBatch.map((lead: any) => {
        ...
        return {
          ...
          phone_number: parsed.phone !== '-' ? parsed.phone : null,
          email: parsed.email !== '-' ? parsed.email : null,
          city: parsed.city !== '-' ? parsed.city : null,
          location: parsed.city !== '-' ? parsed.city : null,
          budget: parsed.budget !== '-' ? parsed.budget : null,
          event_date: parsed.eventDate !== '-' ? parsed.eventDate : null,
          source: lead.campaign_name ? `FB Ads: ${lead.campaign_name}` : `Facebook Ads / ${formName}`,
          status: 'new',
