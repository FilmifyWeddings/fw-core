...
export default function ResetPasswordRootPage() {
  ...
  useEffect(() => {
    // Check for access_token in URL hash or active session
    const checkHashAndSession = async () => {
      // 1. Check if Supabase session is established from hash
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update password using Supabase Auth Client
      ...
      console.error('[Reset Password Root Error]:', err);
