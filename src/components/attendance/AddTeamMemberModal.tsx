...
import { 
  ...
  Trash2, ShieldCheck, Map, Layers, Sun, CheckCircle2, AlertCircle, Lock
} from 'lucide-react';
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100010] flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-sm font-sans overflow-y-auto">
        <motion.div
          ...
          className="bg-[#FFFDF9] rounded-3xl p-6 sm:p-7 max-w-3xl w-full border border-[#EAE5DA] shadow-2xl space-y-5 my-8 max-h-[90vh] overflow-y-auto text-slate-900 relative"
        >
          {/* Sticky Pinned Close Button (never scrolls out of view) */}
          <div className="sticky top-0 z-30 flex justify-end -mt-2 -mr-2 mb-2 pointer-events-none">
            <button
              type="button"
              onClick={onClose}
              className="p-2 text-slate-500 hover:text-slate-800 bg-white/95 hover:bg-slate-100 rounded-xl transition cursor-pointer shadow-md border border-slate-200 pointer-events-auto backdrop-blur-md"
              title="Close"
            >
              <X className="w-5 h-5" />
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 text-xs">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full px-3.5 py-2.5 ${
                        isEditMode 
                          ? 'bg-slate-100 text-slate-500 cursor-not-allowed pr-8 border-slate-200' 
                          : 'bg-[#FAF9F5] text-slate-900 border-[#EAE5DA] focus:ring-2 focus:ring-amber-500/20'
                      } border rounded-xl font-bold focus:outline-none`}
                    />
                    {isEditMode && (
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => !isEditMode && setWhatsappNumber(e.target.value)}
                      className={`w-full px-3.5 py-2.5 ${
                        isEditMode 
                          ? 'bg-slate-100 text-slate-500 cursor-not-allowed pr-8 border-slate-200' 
                          : 'bg-[#FAF9F5] text-slate-900 border-[#EAE5DA] focus:ring-2 focus:ring-amber-500/20'
                      } border rounded-xl font-semibold focus:outline-none`}
                    />
                    {isEditMode && (
                      <Lock className="w-3.5 h-3.5 text-slate-400 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                    )}
                    ...
                    <input
                      ...
                      className={`w-full px-3.5 py-2.5 ${
                        isEditMode 
                          ? 'bg-slate-100 text-slate-500 cursor-not-allowed pr-8 border-slate-200' 
                          : 'bg-[#FAF9F5] text-slate-900 border-[#EAE5DA] focus:ring-2 focus:ring-amber-500/20'
                      } border rounded-xl font-medium focus:outline-none`}
                    />
                    {isEditMode && (
                      <Lock className="w-3.5 h-3.5 text-slate-400 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                    )}
            ...
            <div className="p-5 bg-white rounded-2xl border border-[#EAE5DA] shadow-2xs space-y-3">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                ...
                {isEditMode ? (
                  <span className="flex items-center gap-1 text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200">
                    <Lock className="w-3 h-3 text-slate-400" /> Locked
                  </span>
                ) : (
                  !isAddingNewRole && (
              ...
              {isEditMode ? (
                <div className="relative">
                  <input
                    ...
                    readOnly
                    value={selectedRole}
                    className="w-full px-3.5 py-2.5 bg-slate-100 text-slate-500 border border-slate-200 rounded-xl font-bold cursor-not-allowed pr-8 focus:outline-none"
                  />
                  <Lock className="w-3.5 h-3.5 text-slate-400 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                </div>
              ) : isAddingNewRole ? (
            ...
            <div className="p-5 bg-white rounded-2xl border border-[#EAE5DA] shadow-2xs space-y-3">
              ...
              {/* Office Lat / Lng Coordinates */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                <div>
                  <label className="font-bold text-slate-700 text-xs block mb-1">Office Latitude</label>
                  <input
                    type="number"
                    step="any"
                    ...
                    className="w-full px-3.5 py-2 bg-[#FAF9F5] border border-[#EAE5DA] rounded-xl font-mono font-bold text-slate-900 text-xs focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                  />
                </div>
                <div>
                  <label className="font-bold text-slate-700 text-xs block mb-1">Office Longitude</label>
                  <input
                    type="number"
                    step="any"
