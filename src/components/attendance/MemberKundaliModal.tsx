...
import { 
  analyzeAttendanceRecordTiming, 
  formatMinutesToHumanReadable, 
  getIstTotalMinutes, 
  parseShiftTimeToMinutes 
} from '@/lib/attendance/time-calculations';
...
export default function MemberKundaliModal({
  ...
  const handleMarkRegularized = async (rec: AttendanceRecord) => {
    ...
    try {
      const shiftEndStr = shifts[0]?.end_time || member?.shift_end || '19:00:00';
      const cleanShiftEnd = String(shiftEndStr).length === 5 ? `${shiftEndStr}:00` : String(shiftEndStr);
      const regularizedCheckOut = rec.check_out_time || `${rec.date}T${cleanShiftEnd}+05:30`;

      // Calculate work duration if missing
      let workDuration = rec.work_duration_minutes || rec.total_work_minutes || 0;
      if (!workDuration && rec.check_in_time) {
        const inMins = getIstTotalMinutes(rec.check_in_time) || 0;
        const outMins = getIstTotalMinutes(regularizedCheckOut) || 0;
        if (outMins > inMins) {
          workDuration = outMins - inMins;
        }
      }
      ...
      await onUpdateRecord(rec.id, {
        status: 'present',
        late_minutes: 0,
        check_out_time: regularizedCheckOut,
        work_duration_minutes: workDuration || 480,
        notes: [rec.notes, 'Regularized by Management'].filter(Boolean).join(' | ')
      });

      setFetchedRecords(prev => prev.map(r => r.id === rec.id ? {
        ...r,
        status: 'present',
        late_minutes: 0,
        check_out_time: regularizedCheckOut,
              ...
              {memberRecords.length === 0 ? (
                ...
                <div className="space-y-2.5">
                  {memberRecords.map(rec => {
                    ...
                    const isToday = rec.date === todayDateStr;
                    const isMissedCheckout = !rec.check_out_time && !isToday;

                    let netWork = rec.work_duration_minutes || rec.total_work_minutes || 0;
                    if (isMissedCheckout && netWork === 0 && rec.check_in_time) {
                      const inMins = getIstTotalMinutes(rec.check_in_time);
                      const shiftEndMins = parseShiftTimeToMinutes(shifts[0]?.end_time || member?.shift_end || '19:00', 19 * 60);
                      if (inMins !== null && shiftEndMins > inMins) {
                        netWork = shiftEndMins - inMins;
                      }
                    }
                    ...
                    return (
                      <div 
                        key={rec.id} 
                        className={`p-4 bg-white rounded-2xl border ${isMissedCheckout ? 'border-amber-200/80 bg-amber-50/20' : 'border-[#EAE5DA]'} hover:border-amber-300 shadow-2xs transition-all flex flex-col md:flex-row md:items-center justify-between gap-4`}
                      >
                        {/* 1. Left Column: Date & Day */}
                        <div className="flex items-center gap-3 min-w-[170px]">
                          <div className={`w-10 h-10 rounded-xl ${isMissedCheckout ? 'bg-amber-100 text-amber-900 border-amber-300' : 'bg-amber-50 text-amber-900 border-amber-200'} border flex flex-col items-center justify-center font-black font-mono text-xs shrink-0`}>
                        ...
                        <div className="flex items-center gap-2 min-w-[150px]">
                          {rec.check_out_time ? (
                            ...
                            >
                              ...
                              <div>
                                ...
                                <span className="text-xs font-black text-amber-950 font-mono">
                                  {new Date(rec.check_out_time).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true })}
                                </span>
                            ...
                            <div className="px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-xl text-left flex items-center gap-2 shadow-2xs">
                              <Clock className="w-3.5 h-3.5 text-emerald-600 animate-pulse" />
                              <div>
                                <span className="text-[9px] font-black text-emerald-800 uppercase tracking-wider block">Check-Out</span>
                                <span className="text-xs font-black text-emerald-700 font-mono animate-pulse">
                                  In Progress
                                </span>
                                <span className="text-[9px] font-medium text-emerald-600 block">Active Shift Today</span>
                              </div>
                            </div>
                          ) : (
                            <div className="px-3 py-1.5 bg-rose-50 border border-rose-200 rounded-xl text-left flex items-center gap-2 shadow-2xs">
                              <AlertTriangle className="w-3.5 h-3.5 text-rose-600 shrink-0" />
                              <div>
                                <span className="text-[9px] font-black text-rose-800 uppercase tracking-wider block">Check-Out</span>
                                <span className="text-xs font-black text-rose-700 font-mono">
                                  Missed Check-Out
                                </span>
                                <span className="text-[9px] font-medium text-rose-600 block">No check-out logged</span>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="text-left md:text-right min-w-[150px]">
                          ...
                          <span className="text-[10px] text-slate-500 font-medium block">
                            {pausedWork > 0 ? `${pausedWork}m Paused` : isMissedCheckout ? 'Missed punch-out' : '0m Paused'}
                          </span>
                        </div>

                        {/* 5. Outer Status Tag */}
                        <div className="flex items-center gap-2">
                          {rec.status === 'present' && !recTiming.isLate && !isMissedCheckout && (
                          ...
                          {isMissedCheckout && (
                            <div className="flex items-center gap-1.5">
                              <span className="px-2.5 py-1 rounded-full text-[11px] font-black bg-rose-50 text-rose-800 border border-rose-200">
                                Missed Out
                              </span>
                              {onUpdateRecord && (
                                <button
                                  ...
                                  className="px-2.5 py-1 text-[10px] font-black bg-amber-400 hover:bg-amber-500 text-slate-900 rounded-lg shadow-2xs cursor-pointer transition flex items-center gap-1"
                                  title="Auto-fill shift end check-out time"
                                >
                          ...
                          {(rec.status === 'late' || recTiming.isLate) && !isMissedCheckout && (
                            <div className="flex items-center gap-1.5">
