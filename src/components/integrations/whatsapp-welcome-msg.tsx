...
export function WhatsappWelcomeMsg({ workspaceId, shootType = 'all' }: WhatsappWelcomeMsgProps) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        // 2. Load Welcome Automation Setting from whatsapp_workflow_sequences (persistent category config)
  ...
  const saveSettings = async () => {
    ...
    try {
      if (isDemo) {
        localStorage.setItem(`wa_config_welcome_${workspaceId}_${shootType}`, JSON.stringify({ isActive, steps }));
