...
-- 1. PURGE invalid, corrupt, mock, or empty workspace_id / user_id rows
...
-- 2. Backfill user_id = workspace_id for any valid rows where user_id was unset
UPDATE baileys_sessions
SET user_id = workspace_id
WHERE user_id IS NULL OR user_id = '';

-- 3. Add UNIQUE constraint on user_id (drop old constraint if exists)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 6. Create Strict 1:1 RLS Policies enforcing auth.uid() = user_id
