-- =====================================================================
-- BRAHMASTRA FIX: Sequential Drift Delay Engine
-- Migration: 20260621000001_sequential_delays.sql
--
-- ROOT CAUSE:
--   fn_trigger_whatsapp_workflow was resetting v_scheduled_at to NOW()
--   for every step, causing all steps to schedule concurrently from the
--   ingest time. This means:
--     Step 1: NOW() + 60s
--     Step 2: NOW() + 60s  ← WRONG (concurrent)
--     Step 3: NOW() + 60s  ← WRONG (concurrent)
--
-- FIX:
--   v_scheduled_at is initialized once to NOW() BEFORE the step loop,
--   then each step ACCUMULATES its delay on top of the previous step's
--   v_scheduled_at, creating a true progressive sequential drip chain:
--     Step 1: NOW() + 60s
--     Step 2: Step1_time + 60s  ← Step 1's time + Step 2's delay
--     Step 3: Step2_time + 60s  ← Step 2's time + Step 3's delay
-- =====================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- SEQUENTIAL DRIFT FIX:
    -- Initialize baseline ONCE before the step loop.
    -- Each step will ADD its own delay on top of the PREVIOUS step's scheduled time.
    -- This ensures: Step1=T+d1, Step2=T+d1+d2, Step3=T+d1+d2+d3 (true drip chain)
    v_scheduled_at := NOW();
...
-- Same sequential drift fix applied to the manual "Sync Staged Group" RPC.
...
BEGIN
  ...
  LOOP
    ...
    -- SEQUENTIAL DRIFT FIX: Initialize baseline ONCE per lead before step loop
    v_scheduled_at := NOW();
