 ...
 */
async function upsertWithRetry(supabase, workspaceId, payload, maxRetries = 3) {
    let attempt = 0;
    let lastError = null;
    while (attempt < maxRetries) {
        ...
        try {
            ...
            logger.warn({ attempt, maxRetries, code: error.code, message: error.message, workspaceId }, isTimeout
...
export async function useSupabaseAuthStateNamespaced(supabase, targetWorkspaceId, userId) {
    ...
    async function persistAuthStateInternal() {
        if (saveTimer) {
            clearTimeout(saveTimer);
            saveTimer = null;
        }
    ...
    async function saveCredsPublic() {
        if (saveTimer) {
            clearTimeout(saveTimer);
            saveTimer = null;
        }
