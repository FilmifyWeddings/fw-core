...
async function sendTemplateMessage(to, templateId, variables, wsId = WORKSPACE_ID) {
    ...
    if (body) {
        const replaceFn = (match, rawKey) => {
            const key = rawKey.trim();
            const normalizedKey = key.toLowerCase();
            // 1. Time / Date
            if (normalizedKey === 'created_time' || normalizedKey === 'timestamp') {
                if (variables.created_time)
                    return String(variables.created_time);
            ...
            if (Array.isArray(variables.field_data)) {
                for (const item of variables.field_data) {
            ...
            if (variables.raw_payload && typeof variables.raw_payload === 'object') {
                const rawKeys = Object.keys(variables.raw_payload);
                for (const alias of [normalizedKey, ...aliases]) {
                    const found = rawKeys.find(k => k.toLowerCase() === alias.toLowerCase());
                    if (found && variables.raw_payload[found] != null && String(variables.raw_payload[found]).trim() !== '') {
                        return String(variables.raw_payload[found]);
                    }
                }
            }
            return match;
...
async function executeAction(action) {
    ...
    let targetWsId = action.workspace_id ||
        action.user_id ||
        action.workspaceId ||
        action.userId ||
 ...
 */
async function syncOpenSessionToDb(wsId, sock, authState) {
    if (!wsId || wsId.trim() === '' || wsId === 'null' || wsId === 'undefined')
        return;
    let phoneNum = null;
    const rawJid = sock?.user?.id || sock?.user?.jid || authState?.state?.creds?.me?.id || authState?.state?.creds?.me?.jid;
    if (rawJid) {
...
async function main() {
     ...
     */
    async function runSelfHealingSessionSync() {
        for (const [wsId, sess] of activeSessions.entries()) {
            if (!wsId || wsId.trim() === '' || wsId === 'null' || wsId === 'undefined')
                continue;
            const hasUser = !!(sess.sock?.user?.id || sess.sock?.user?.jid);
            if (hasUser) {
                try {
                    const { data: dbSess } = await supabase
                        .from('baileys_sessions')
